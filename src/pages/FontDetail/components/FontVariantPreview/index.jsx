...
const FontVariantPreview = ({ previewText, fontFamily, variant, category, fontSize }) => {
  ...
  useEffect(() => {
    setIsSelected(fontFamily in collection && collection[fontFamily]['variants'].includes(variant))
  ...
  return (
    <div className="py-12 w-full relative text-zinc-800 first:border-t-2 border-b-2">
      ...
      <h3
        className="overflow-hidden mr-[100px] laptop:mr-[200px]  whitespace-nowrap"
        style={{ fontSize, fontFamily, fontStyle, fontWeight }}
      >
        {previewText}
      </h3>
      <EButton
        className={`absolute right-0 top-1/2 -translate-y-1/2 font-medium 
