...
const MultipleSelector = ({
  ...
  return (
    <div className="relative w-fit" ref={wrapperRef}>
      <EButton
        className={`flex items-center justify-between p-2 text-sm min-w-[80px] hover:bg-slate-100 border ${buttonClassName}`}
      ...
      >
        <ul className="flex items-center gap-2 justify-start pr-4">
          {selectedIds.length === items.length
            ...
            ? noSelectedPromp
            : selectedIds.map((id) => (
                <span className="bg-blue-400 text-white px-2" key={id}>
      ...
      {showOptions && (
        <ul
          className={`bg-white shadow-md min-w-[100px] flex flex-col absolute left-0 top-10 overflow-y-auto z-20 ${className}`}
          {...props}
        >
