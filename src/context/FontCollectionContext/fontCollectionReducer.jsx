    ...
    case 'toggle': {
      ...
      if (family in collection) {
        if (collection[family]['variants'].includes(variant)) {
          collection[family]['variants'] = collection[family]['variants'].filter(
            (v) => v !== variant
          )
          ...
          collection[family]['variants'].sort((lVariant, rVariant) => {
            const { fontWeight: leftFontWeight, fontStyle: leftFontStyle } =
              extractVariantInfo(lVariant)
            const { fontWeight: rightFontWeight } = extractVariantInfo(rVariant)
            if (leftFontWeight === rightFontWeight) return leftFontStyle ? 1 : 0
